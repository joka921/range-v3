...
void test_case_from_std_array_constructor()
{
    ...
    {
        span<int> s{arr};
        CHECK((s.size() == narrow_cast<std::size_t>(arr.size()) && s.data() == arr.data()));

        span<const int> cs{arr};
        CHECK((cs.size() == narrow_cast<std::size_t>(arr.size()) && cs.data() == arr.data()));
    }
...
void test_case_first()
{
    ...
    {
        span<int, 5> av = arr;
        CHECK(av.first<2>().size() == 2u);
        CHECK(av.first(2).size() == 2u);
    }

    {
        span<int, 5> av = arr;
        CHECK(av.first<0>().size() == 0u);
        CHECK(av.first(0).size() == 0u);
    }

    {
        span<int, 5> av = arr;
        CHECK(av.first<5>().size() == 5u);
        CHECK(av.first(5).size() == 5u);
    }
...
void test_case_last()
{
    ...
    {
        span<int, 5> av = arr;
        CHECK(av.last<2>().size() == 2u);
        CHECK(av.last(2).size() == 2u);
    }

    {
        span<int, 5> av = arr;
        CHECK(av.last<0>().size() == 0u);
        CHECK(av.last(0).size() == 0u);
    }

    {
        span<int, 5> av = arr;
        CHECK(av.last<5>().size() == 5u);
        CHECK(av.last(5).size() == 5u);
    }
...
void test_case_subspan()
{
    ...
    {
        span<std::size_t, 5> av = arr;
        CHECK((av.subspan<2, 2>().size() == 2));
        CHECK(av.subspan(2, 2).size() == 2u);
        CHECK(av.subspan(2, 3).size() == 3u);
    }

    {
        span<std::size_t, 5> av = arr;
        CHECK((av.subspan<0, 0>().size() == 0u));
        CHECK(av.subspan(0, 0).size() == 0u);
    }

    {
        span<std::size_t, 5> av = arr;
        CHECK((av.subspan<0, 5>().size() == 5u));
        CHECK(av.subspan(0, 5).size() == 5u);
    }

    {
        span<std::size_t, 5> av = arr;
        CHECK((av.subspan<4, 0>().size() == 0u));
        CHECK(av.subspan(4, 0).size() == 0u);
        CHECK(av.subspan(5, 0).size() == 0u);
    }
    ...
    {
        span<std::size_t> av;
        CHECK(av.subspan(0).size() == 0u);
    }

    {
        span<std::size_t> av = arr;
        CHECK(av.subspan(0).size() == 5u);
        CHECK(av.subspan(1).size() == 4u);
        CHECK(av.subspan(4).size() == 1u);
        CHECK(av.subspan(5).size() == 0u);
        const auto av2 = av.subspan(1);
        for (std::size_t i = 0; i < 4; ++i) CHECK(av2[i] == i + 2);
    }
    ...
    {
        span<std::size_t, 5> av = arr;
        CHECK(decltype(av.subspan<0, dynamic_extent>())::extent == 5u);
        CHECK((av.subspan<0, dynamic_extent>().size() == 5));
        CHECK(decltype(av.subspan<1, dynamic_extent>())::extent == 4u);
        CHECK((av.subspan<1, dynamic_extent>().size() == 4));
        CHECK(decltype(av.subspan<2, dynamic_extent>())::extent == 3u);
        CHECK((av.subspan<2, dynamic_extent>().size() == 3));
        CHECK(decltype(av.subspan<3, dynamic_extent>())::extent == 2u);
        CHECK((av.subspan<3, dynamic_extent>().size() == 2));
        CHECK(decltype(av.subspan<4, dynamic_extent>())::extent == 1u);
        CHECK((av.subspan<4, dynamic_extent>().size() == 1));
        CHECK(decltype(av.subspan<5, dynamic_extent>())::extent == 0u);
